    ...
    <div className='w-full bg-white py-8'>
      <div className='max-w-screen-xl mx-auto px-4'>
        ...
        <div className='mb-6'>
          ...
          <div className='h-1 w-24 bg-orange-400 mt-1 rounded'></div>
        </div>
          ...
          {display.length === 0 ? (
            ...
            display.map((product, index) => {
              ...
              return (
                <div
                  key={product.id}
                  className='group border rounded-xl bg-white shadow-md hover:shadow-lg transition-all duration-200 p-4 flex flex-col items-center'
                >
                  <div className='relative w-36 h-40 mb-3 flex items-center justify-center'>
                    <Image
                      src={product.image}
                      alt={product.name}
                      width={144}
                    ...
                    <div className='absolute -bottom-2 left-1/2 transform -translate-x-1/2 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-200'>
                      ...
                      <button
                        ...
                        title={favorites[product.id] ? 'Remove from Favorites' : 'Add to Favorites'}
                      >
                        {favorites[product.id] ? (
